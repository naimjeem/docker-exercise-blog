...
describe('Posts Integration Tests', () => {
  ...
  beforeAll(async () => {
    // Skip integration tests if no database is available (Windows runners)
    if (process.env.SKIP_INTEGRATION_TESTS === 'true') {
      console.log('Skipping integration tests - no database available');
      return;
    }
  ...
  describe('POST /api/posts', () => {
    it('should create a post and return it', async () => {
      if (process.env.SKIP_INTEGRATION_TESTS === 'true') {
        console.log('Skipping integration test - no database');
        return;
      }
      const newPost = {
      ...
      const dbResult = await pool.query('SELECT * FROM posts WHERE id = $1', [
        response.body.id
      ]);
  ...
  describe('GET /api/posts/:id', () => {
    ...
    it('should return 404 for non-existent post', async () => {
      ...
      const response = await request(app).get('/api/posts/99999').expect(404);
